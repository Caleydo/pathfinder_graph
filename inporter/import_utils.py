...
def create_edge_query(type, source_node_id, target_node_id, properties, source_type='_Network_Node', update_only=False):
  merge = u'MATCH (source:{0} {{id:"{1}"}}) MATCH (target:_Network_Node {{id:"{2}"}}) MERGE (source)-[el:{3}]->(target) '.format(
    source_type, source_node_id, target_node_id, type)
  update = u'MATCH (source:{0} {{id:"{1}"}})-[el:{3}]->(target:_Network_Node {{id:"{2}"}}) '.format(source_type,
                                                                                               source_node_id,
                                                                                               target_node_id, type)
