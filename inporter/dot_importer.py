    ...
    for i,row in enumerate(csv.reader(f, delimiter='\t')):
      ...
      try:
        ...
        for subids in grouper_it(100, geneids):
          q = u'MATCH (source:Pathway {{id:"{0}"}}), (target:_Network_Node ) WHERE target.geneids in ["{1}"] CREATE source-[el:ConsistsOfEdge]->target'.format(
            id+'_'+category, u'","'.join(subids))
          qs.append(q)
...
def import_pheno():
  with open(args.data_dir+'mousePhen.humanGenes.tsv', 'r') as f:
    for row in (r for i, r in enumerate(csv.reader(f, delimiter='\t')) if i > 0):
      ...
      try:
        q = u'MATCH (source:Phenotype {{id:"{0}"}}), (target:_Network_Node {{geneids:"{1}"}}) CREATE source-[el:ConsistsOfEdge]->target'.format(
          pheno, geneid)
        importer.append(q)
      except:
...
def import_disease():
  with open(args.data_dir+'humandisease.txt', 'r') as f:
    for row in (r for i, r in enumerate(csv.reader(f, delimiter='\t')) if i > 0):
      ...
      importer.append(
        u'MATCH (source:Disease {{id:"{0}"}}), (target:_Network_Node {{geneids:"{1}"}}) CREATE source-[el:ConsistsOfEdge]->target'.format(
          disease, geneid))
