...
def importfile(file):
  ...
  for key, node in g['nodes'].iteritems():
    ...
    color = node['color']
    label = node['label']
    add_node(tx, ['_Network_Node'], id, dict(idType=targetclass,color=color,name=label,geneids=geneids,pathways=[targetclass]))
  ...
  for k, g in itertools.groupby(sorted(g['nodes'].iteritems(), key=lambda x : x[1]['targetclass']), lambda x : x[1]['targetclass']):
